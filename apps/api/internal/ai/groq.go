...
func NewGroqProvider(cfg *config.Config) *GroqProvider {
	clientConfig := openai.DefaultConfig(cfg.AI.GroqKey)
...
func (p *GroqProvider) ChatCompletion(ctx context.Context, req ChatRequest) (ChatResponse, error) {
	...
	if err != nil {
		metrics.AIRequestErrors.WithLabelValues("groq").Inc()
		return ChatResponse{}, fmt.Errorf("groq completion: %w", err)
	}
	metrics.AIRequestLatency.WithLabelValues("groq").Observe(latency)
	metrics.AITokensUsed.WithLabelValues("groq", "input").Add(float64(resp.Usage.PromptTokens))
	metrics.AITokensUsed.WithLabelValues("groq", "output").Add(float64(resp.Usage.CompletionTokens))
...
func (p *GroqProvider) StreamCompletion(ctx context.Context, req ChatRequest, callback func(token string)) error {
	msgs := make([]openai.ChatCompletionMessage, 0, len(req.Messages)+1)
	if req.SystemPrompt != "" {
		msgs = append(msgs, openai.ChatCompletionMessage{
			Role:    openai.ChatMessageRoleSystem,
...
func (p *GroqProvider) ClassifyIntent(ctx context.Context, message string) (string, error) {
